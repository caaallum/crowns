...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[]) {
    ...
    SDL_Window* window = SDL_CreateWindow("Crowns", windowStartWidth, windowStartHeight, SDL_WINDOW_RESIZABLE | SDL_WINDOW_HIGH_PIXEL_DENSITY);
    if (not window){
        return SDL_Fail();
    }
    ...
    Level l = level_generate(5, 5);
    int width, height;
    SDL_GetWindowSize(window, &width, &height);
    auto crown_path = basePath / "crown.png";
    SDL_Texture *crown = IMG_LoadTexture(renderer, crown_path.string().c_str());
    ...
    *appstate = new AppContext{
       ...
       .renderer = renderer,
       .track = mixerTrack,
        .grid = new Grid(l, crown, (width / 5) - 5),
    };
